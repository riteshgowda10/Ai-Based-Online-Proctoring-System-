        ...
        try:
            ...
            if output_format.lower() == 'pdf':
                try:
                    ...
                    config = pdfkit.configuration(wkhtmltopdf=pdfkit_config) if (pdfkit_config and os.path.exists(pdfkit_config)) else None
                    pdfkit.from_string(html_content, output_path, options=options, configuration=config)
                except Exception as pdf_err:
                    self.logger.warning(f"PDF generation failed ({pdf_err}). Falling back to HTML report format.")
