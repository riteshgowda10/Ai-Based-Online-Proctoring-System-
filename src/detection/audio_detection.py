    ...
    def _run(self):
        ...
        if not PYAUDIO_AVAILABLE:
            if self.alert_logger:
                self.alert_logger.log_alert("AUDIO_INFO", "PyAudio module not loaded; audio monitoring standby.")
            return
        ...
        try:
            ...
            if self.alert_logger:
                self.alert_logger.log_alert("AUDIO_ERROR", f"Audio initialization failed: {str(e)}")
            return
        ...
        try:
            ...
            if self.alert_logger:
                self.alert_logger.log_alert("AUDIO_ERROR", f"Audio stream error: {str(e)}")
        finally:
            try:
                stream.stop_stream()
                stream.close()
